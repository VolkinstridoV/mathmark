...
CSS = b"""
.koren-title { font-weight: 700; }
.koren-dim { opacity: 0.62; font-size: 0.86em; }
.koren-progress trough { min-height: 4px; }
.koren-progress progress { min-height: 4px; background: linear-gradient(90deg, #7C3AED, #A855F7); }
.koren-hit { font-family: monospace; font-size: 0.82em; opacity: 0.7; }
row.koren-row { padding: 6px 4px; }
"""
...
class KorenWindow(Adw.ApplicationWindow):
    ...
    def _content(self) -> Gtk.Widget:
        ...
        ucm.register_script_message_handler("koren", None)
        ucm.connect("script-message-received::koren", self._on_message)
    ...
    def _row_shell(self, glyph: str, title: str, subtitle: str) -> tuple[Gtk.ListBoxRow, Gtk.Box]:
        ...
        lbl.add_css_class("koren-title")
        sub = Gtk.Label(label=subtitle, xalign=0, ellipsize=3)
        sub.add_css_class("koren-dim")
        col.append(lbl)
        col.append(sub)
        box.append(col)
        row.set_child(box)
    ...
    def _render(self) -> None:
        ...
        if self.current is None:
            ...
            self.toc_btn.set_sensitive(False)
            return
        self._js(f"Koren.render({json.dumps(self.text)});")
    ...
    def set_scale(self, value: float) -> None:
        ...
        self.st.save()
        self._js(f"Koren.setScale({self.st.scale:g});")
