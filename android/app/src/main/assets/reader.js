...
(function () {
  ...
  Koren.render = function (src) {
      ...
      if (ITEM.test(srcLines[i]) && pairs(srcLines[i].match(ITEM)[2], srcLines[i].match(ITEM)[4])) {
        ...
        while (i < lines.length) {
          ...
          li.push(
            ...
            '<span class="box ' + kind + '" data-off="' + off + '">' + CHECK + '</span>' +
    ...
    if (window.Android && Android.onToc) Android.onToc(JSON.stringify(toc));
  };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (isNaN(off)) return;
    if (window.Android && Android.onCycle) Android.onCycle(off);
  });
})();
